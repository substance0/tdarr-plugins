...
const getAudioCodecInfo = (ffProbeData, label = null) => {
...
const getMediaStreamInfo = (
  ...
  if (fileInfo.ffProbeData?.streams) {
    ...
    // For transcode_success, show both original and transcoded audio tracks
    if (
      notificationType === "transcode_success" &&
      originalFileInfo?.ffProbeData?.streams
    ) {
      const originalAudioInfo = getAudioCodecInfo(
        originalFileInfo.ffProbeData,
        "Original Audio:",
      );
      const transcodedAudioInfo = getAudioCodecInfo(
        fileInfo.ffProbeData,
        "🆕 Transcoded Audio:",
      );

      if (originalAudioInfo && transcodedAudioInfo) {
        details.push(originalAudioInfo);
        details.push(transcodedAudioInfo);
      } else if (transcodedAudioInfo) {
        details.push(`Audio Tracks:${transcodedAudioInfo}`);
      }
    } else {
      // For other notification types, show current audio tracks only
...
const buildNotificationDetails = (
  ...
  return [
    ...getFileBasicInfo(fileInfo),
    ...getMediaStreamInfo(fileInfo, originalFileInfo, notificationType),
  ];
};
...
const buildNotificationContent = (
  ...
  const notificationDetails = buildNotificationDetails(
    fileInfo,
    originalFileInfo,
    notificationType,
  );
...
const plugin = async (args) => {
  ...
  try {
    ...
    const originalFileInfo = args.originalLibraryFile || null;
